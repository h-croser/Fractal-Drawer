...
from view.gui.widgets.CustomWidgetManager import CustomWidgetManager
...
class FractalViewWrapper:
    def __init__(self, render_trigger: Callable, layer_setter: Callable, layer_getter: Callable,
        ...
        # Set defaults
        self.figure_background_color: list[float] = FIGURE_BACKGROUND_COLOR
        self.axes_background_color: list[float] = AXES_BACKGROUND_COLOR
        self.branch_start_color: list[float] = BRANCH_START_COLOR
        self.branch_end_color: list[float] = BRANCH_END_COLOR
        self.leaf_color: list[float] = LEAF_NODE_COLOR
        ...
        # Set figure
        self.parent_fig: Figure = plt.figure(figsize=(FIGURE_SIZE, FIGURE_SIZE), facecolor=self.figure_background_color)
        gs: GridSpec = GridSpec(2, 1, height_ratios=[3, 1])
        self.plot_fig: SubFigure = self.parent_fig.add_subfigure(gs[0], facecolor=self.axes_background_color)
    ...
    def render(self, tree: Tree):
        self.ax.set_facecolor(self.axes_background_color)
        self.ax.cla()
